...
3. Work out his per-90 stats and ratios, with weights set for each role group. The weights lean on the stats that reflect lasting ability:
   ...
   - **Keeper rank leans on club level.** Even after all that, regular Premier League keepers sit within 6.8–7.05, a gap that's mostly noise. Stretching it over the whole scale gave near-random ranks (Henderson 37, Kelleher 67). So a keeper's rank is 100 × club rank ÷ 1200 − 10, plus 0.2 × (rating percentile − 50). Club level carries it, since good clubs sign good keepers, and the rating moves it by up to about ±10. The keeper's club level is smoothed over this and earlier seasons, with weight 0.5 ^ years apart, so a move to a bigger club lifts him over a season or two rather than overnight. Suzuki went Parma → Aston Villa: 66 → 78, not 83. A keeper who plays under 80% of his club's minutes is scaled down by up to 20%, so a backup at a top club isn't rated elite. It's measured as a share of the games played so far, so it doesn't penalise anyone early in the season.
...
7. **Club level first, stats adjust it** (all players): rank = 100 × club rank ÷ 1200 − 12, plus 0.3 × (stats percentile − 50). Club level sets the base, because being a regular for a strong club is good evidence of quality, and stats move a player by up to about ±15 (keepers ±10, since their stats are noisier). The club rank is the average rank, at the time, of the clubs he played for, smoothed over this and earlier seasons so a transfer lifts him gradually. A player under 70% of his club's minutes (80% for keepers) is scaled down by up to 20%, so squad players don't rate like starters. An estimated season at a club we track players for, where he didn't play, gets the full markdown, and one with no club data takes the share from his nearest real season. For example, Tah's below-average 22/23 at Leverkusen (1006) is 78.
...
Line-up roles are stored in `fixture_players.role` and `fixture_players.grid`, and formations in `fixture_formations`. The site shows player ranks on the Rankings tab (Players view, filterable by role group). Each row has the player's club badge and photo, with his club and nationality under his name. An age range slider above the league filter narrows the list to an age range, and the counts in the filter follow it. The table shows each player's age and his **season ranks** for 26/27 back to 21/22, sorted by the current season: worked out from **that season's matches only** (`player_season_ranks`). His season totals get the same stat score as above, turned into a percentile among player-seasons with 900+ minutes in the same role group, then multiplied by his clubs' average rank that season ÷ 1200. Early in a season the score leans on his previous season (weighted as 15 full games at the start, fading to nothing by the end of the season), so two good games don't make a player the best in the world. A season with few minutes is filled in from the player's nearest season with 1,500+ minutes, walked through the typical **age curve**. The curve is measured from players with 1,500+ minutes in consecutive seasons: about +0.1 to +0.2 a year from 18 to 21, flat from 22 to 27, falling from about 28 and fastest at 32–34. Below 18 there are too few regulars to measure the curve, so it's set by hand to steepen: +0.15 a year at 17, and 0.05 more for each year younger (0.30 at 14). Teenagers drop off quickly going back, so Lamine Yamal reads 63 at 14 and 78 at 15, against 84 in his first full season. A player with no well-measured season anywhere is marked down instead. **Every season from 21/22 to now has a number for every player.** A season with no minutes in these leagues is estimated. That can be a year in a league without player data, the years before his debut here, or the current season if he hasn't played yet. If he has no season with 1,500+ minutes, the estimate starts from his nearest season, whatever its minutes. It gets the age-curve estimate on its own. His club that season comes from API-Football's list of the clubs a player has been at (`player_career_teams`, fetched once per player with a gap, and by the nightly job for new ones). Only clubs we have league fixtures for that season are used, so the club's real rank then sets its level. For example, Joan García's 23/24 is Espanyol in the Segunda, at 878. Otherwise the club level comes from the seasons either side. The hover names the club. It's stored with 0 minutes and shown on the site as an outlined chip in italics. The top 10% of the scale is spread by how far a player's score is above the 90th percentile, up to the best seasons on record, so the very best stand apart instead of all sitting at about 99. A season is blank if he didn't play in these leagues. Hovering a season cell shows his age that season: his age now for the current season, and one less for each season before. Then, for each club he played for that season, it shows the club's average rank over his matches, followed by his minutes, average match rating, goals and assists. That detail is in `docs/data/player_seasons.json`, loaded only when the Players view opens. It also shows the predicted XI for a team's next match in the team pop-up, and the XI ratings on match cards.
